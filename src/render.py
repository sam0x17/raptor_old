...
print()
print("RAPTOR2 Blender hook loaded")
...
def get_pointcloud():
	...
	for obj in bpy.context.selected_objects:
		for vert in obj.data.vertices:
			pcloud.append(obj.matrix_world * vert.co)
			#print(str(pcloud[len(pcloud) - 1]))
	return pcloud
...
def get_3d_covariance_matrix(pcloud):
	...
	for vert in pcloud:
		inner = vert - mean
		out = outer3d(inner, inner)
		cov = cov + out
	cov = cov * (1.0 / (len(pcloud) - 1))
	return cov
...
#java -cp bin:lib/imgscalr-lib-4.2.jar:lib/jblas-1.2.3.jar raptor.planB.GenTrainingB
	subprocess.call(['java', '-cp', '.:lib:bin:../lib/imgscalr-lib-4.2.jar:../lib/jblas-1.2.3.jar', '-Xmx2500M', '-Xms2500M', 'raptor.plan' + plan + '.GenTraining' + plan, img_filename, annotation_filename,
